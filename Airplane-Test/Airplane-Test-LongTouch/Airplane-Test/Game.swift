...
extension ViewController {
    ...
    func configureController() {
        ...
        let upLongCatcher = UILongPressGestureRecognizer(target: self, action: #selector(upLongTouch))
        upLongCatcher.minimumPressDuration = 0.1
        upButton.addGestureRecognizer(upLongCatcher)

        let downLongCatcher = UILongPressGestureRecognizer(target: self, action: #selector(downLongTouch))
        downLongCatcher.minimumPressDuration = 0.1
        downButton.addGestureRecognizer(downLongCatcher)
...
//        let dragField = UIFieldBehavior.dragField()
        print(location)
    }
}
